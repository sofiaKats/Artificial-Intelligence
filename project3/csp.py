...
class CSP(search.Problem):
    ...
    def __init__(self, variables, domains, neighbors, constraints, constraint_tuples):
        ...
        self.nassigns = 0
        self.constraint_tuples = constraint_tuples

        #for dom/wdeg
        # Dictionary storing the weights of each tuple that participates in constraints
        self.weight = {}
        for tupl in constraint_tuples:
            self.weight[tupl] = 1 
        #    FROM GIVEN PDF
        # Then,in order to benefit,at the beginning of the search,from relevant information
        # about current variable degrees,we propose to initialize all weight counters to 1. 
...
##########################################################################################
    def my_display(self):
        ...
        for sub in self.variables:
            for i in range(0, len(sub), chars_per_line):
                print(sub[i:i+chars_per_line])
            day_hour = {sub: self.curr_domains[sub][0] for v in self.variables if v==sub and 1 == len(self.curr_domains[sub])}

            if(day_hour[sub][0] < 13):
                print("			", end=" ")
                for d in range(1,day_hour[sub][0]):
                    print("    \t", end="   ")
                print("DAY ",day_hour[sub][0])
                for d in range(1,day_hour[sub][0]):
                    print("    \t", end="   ")
                print("\t\t\t",day_hour[sub][1])
            print("_________________________")
        ...
        for sub in self.variables:
            ...
            if(day_hour[sub][0] >= 13):
                print("			", end=" ")
                for d in range(13,day_hour[sub][0]):
                    print("    \t", end="   ")
                print("DAY ",day_hour[sub][0])
                for d in range(13,day_hour[sub][0]):
                    print("    \t", end="   ")
        ...
        for day in range(1,22):
            ...
            for sub in self.variables:
                ...
                if(day_hour[sub][0] == day):
                    if(day_hour[sub][1] == "9:00-12:00"):
                        lesson1 = sub
                    if(day_hour[sub][1] == "12:00-3:00"):
                       lesson2 = sub
                    if(day_hour[sub][1] == "3:00-6:00"):
                        lesson3 = sub
            chars_per_line = 23
...
def dom_wdeg(assignment, csp):
    return argmin_random_tie([v for v in csp.variables if v not in assignment],
                             key=lambda var: domain_weight_degree_ratio(csp, var, assignment))

def domain_weight_degree_ratio(csp, subject, assignement):
    total_weight  = 0
    domain_length = 0
    if (csp.curr_domains != None):
        domain_length = len(csp.curr_domains[subject])
    else:
        domain_length = len(csp.domains[subject])

    # each variable can be evaluated while taking into account its neighborhood.

    # for each neighbor that isn't assigned find the total weight
    for neighbor in csp.neighbors[subject]:
        if neighbor not in assignement:
            total_weight = csp.weight[(subject,neighbor)]

    #combining weighted degrees and domain sizes yields dom_wdeg
    #a heuristic that selects first the variable with the smallest 
    # ratio current domain size to current weighted degree.
    if(total_weight == 0):
        total_weight = 1
    return (domain_length / total_weight )
