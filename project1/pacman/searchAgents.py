...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        space)
        """
        # start state is a tuple including the current position and a tuple containing
    ...
    def isGoalState(self, state):
        ...
        """
        # the goal state is to have visited all 4 corners
    ...
    def expand(self, state):
        ...
        for action in self.getActions(state):
            ...
            if nextState in self.corners and nextState not in suc_corners:
                    suc_corners+=(nextState,)

            cost = self.getActionCost(state, action, nextState)
            children.append( ((nextState, suc_corners), action, cost) )
...
def cornersHeuristic(state, problem):
    ...
    node = state[0] # fetch current position
    visited_corners = state[1] #fetch list of visited corners
    notVisited = []

    # the heuristic must take int account only the nonVisited corners so we find them first
    ...
    while len(notVisited) !=0:
        # dictionary holds corner as key and distance as value (corner: distance)
        manhattan_distances = {} 
        for corner in notVisited:
            distance = util.manhattanDistance(node, corner)
            manhattan_distances[corner] = distance 

        # find minimum value in dictionary and it's corresponding key
        min_dist = min(manhattan_distances.values())
        node = min(manhattan_distances.keys(), key=(lambda k: manhattan_distances[k]))
...
def foodHeuristic(state, problem):
    ...
    # We calculate the maze distance from starting position to every single non-wall state
    for food in foodList:
        ...
        if distance > heuristic_value: # We keep the longest distance
